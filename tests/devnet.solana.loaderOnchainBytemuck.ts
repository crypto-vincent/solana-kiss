...
it("run", async () => {
  ...
  const decoded = jsonCodec.decoder(accountInfo.state);
  expect(decoded.state.metadata.vocabSize).toStrictEqual(129280n);
  expect(decoded.state.coordinator.config.minClients).toStrictEqual(24);
});
        ...
        LLM: jsonCodecArrayToTuple(
          jsonCodecObject({
            ...
            checkpoint: jsonCodecObjectToEnum({
              Ephemeral: jsonCodecConst(undefined),
            ...
            dataLocation: jsonCodecObjectToEnum({
              Dummy: jsonCodecConst(undefined),
              ...
              Http: jsonCodecArrayToTuple(
                jsonCodecObject({
                  ...
                  shuffle: jsonCodecObjectToEnum({
                    DontShuffle: jsonCodecConst(undefined),
