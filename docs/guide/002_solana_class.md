...
`Solana` is the main entry point. It wraps an RPC client and an IDL loader, and
exposes everything you need for the most common on-chain operations.
...
confirmation in a single call. It returns once the transaction is confirmed (or
throws if the block hash expires).
...
```ts
const { executionReport } = await solana.prepareAndSimulateTransaction(signer, [
  instructionRequest,
]);
```
...
```ts
const pdaAddress = await solana.findPdaAddress(programAddress, "userAccount", {
  user: walletAddress,
});
```
